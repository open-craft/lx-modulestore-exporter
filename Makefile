...
# PROJECT_ROOT is usually '/edx/src/lx-modulestore-exporter/' on an Open edX devstack
...
quality: ## check coding style with pycodestyle and pylint
	pylint lx_modulestore_exporter $(PROJECT_ROOT)setup.py
	pylint --py3k lx_modulestore_exporter $(PROJECT_ROOT)setup.py
	pycodestyle $(PROJECT_ROOT)lx_modulestore_exporter $(PROJECT_ROOT)setup.py
	isort --check-only --diff --recursive $(PROJECT_ROOT)lx_modulestore_exporter $(PROJECT_ROOT)setup.py
...
test: clean
	python -m pytest --ds=cms.envs.test --no-cov --nomigrations $(PROJECT_ROOT)lx_modulestore_exporter/tests/
...
extract_translations: ## extract strings to be translated, outputting .mo files
	rm -rf docs/_build
	cd lx-modulestore-exporter && ../manage.py makemessages -l en -v1 -d django
	cd lx-modulestore-exporter && ../manage.py makemessages -l en -v1 -d djangojs

compile_translations: ## compile translation files, outputting .po files for each supported language
	cd lx-modulestore-exporter && ../manage.py compilemessages

detect_changed_source_translations:
	cd lx-modulestore-exporter && i18n_tool changed
...
dummy_translations: ## generate dummy translation (.po) files
	cd lx_modulestore_exporter && i18n_tool dummy
